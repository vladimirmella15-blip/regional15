...
export default function EventosPageGrid({ eventos }: EventosPageGridProps) {
  if (!eventos || eventos.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 30px', background: 'linear-gradient(135deg, #f8fafb, #f0f4f9)', borderRadius: '16px', border: '2px dashed rgba(0,56,118,0.12)' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '1rem' }}>No hay eventos disponibles en este momento.</p>
      </div>
    )
  }

  return (
    <div className="eventos-page-grid">
      {eventos.map((item) => (
        <div className="event-card-v2" key={item.id}>
